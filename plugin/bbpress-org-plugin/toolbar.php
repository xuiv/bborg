...
function bbporg_new_admin_bar_site_menu( $wp_admin_bar ) {
	...
	$wp_admin_bar->add_menu( array(
		'id'    => 'bbp-site-name',
		'title' => __( 'Home', 'bbpress-org-plugin' ),
		'href'  => home_url( '/' )
	) );
	...
	if ( is_user_logged_in() ) {
		...
		$wp_admin_bar->add_menu( array(
			'id'     => 'bbp-new-topic',
			'title'  => __( 'New Topic', 'bbpress-org-plugin' ),
		...
		if ( is_super_admin() ) {
			...
			$wp_admin_bar->add_menu( array(
				...
				'id'     => 'bbp-admin-link',
				'title'  => __( 'Admin Dashbooard', 'bbpress-org-plugin' ),
				'href'   => get_admin_url()
			) );
		}
	...
	// Not logged in
		...
		$wp_admin_bar->add_menu( array(
			'id'     => 'bbp-login',
			'title'  => __( 'Log in', 'bbpress-org-plugin' ),
			'href'   => '/login/'
		) );
	}
}
...
function bbporg_admin_bar_my_account_item( $wp_admin_bar ) {
	...
	if ( is_user_logged_in() ) {
		...
		$wp_admin_bar->add_menu( array(
			...
			'meta'      => array(
				'class'     => $class,
				'title'     => __('My Account', 'bbpress-org-plugin'),
			),
		) );
	} else {
		$avatar = get_avatar( 0, 16, 'mystery' );
		$howdy  = __( 'Anonymous', 'bbpress-org-plugin' );
		$class  = empty( $avatar ) ? '' : 'with-avatar';
...
function bbporg_admin_bar_my_account_menu( $wp_admin_bar ) {
	...
	if ( is_user_logged_in() ) {
		...
		$wp_admin_bar->add_menu( array(
			'parent' => 'user-actions',
			'id'     => 'edit-profile',
			'title'  => __( 'Edit My Profile', 'bbpress-org-plugin' ),
			'href' => $profile_url,
		) );
		$wp_admin_bar->add_menu( array(
			'parent' => 'user-actions',
			'id'     => 'logout',
			'title'  => __( 'Log Out', 'bbpress-org-plugin' ),
			'href'   => wp_logout_url(),
		) );
	...
	// Anonymous
		...
		$wp_admin_bar->add_menu( array(
			'parent' => 'user-actions',
			'id'     => 'register',
			'title'  => __( 'Register', 'bbpress-org-plugin' ),
			'href'   => '/wp-login.php?action=register'
		) );
		$wp_admin_bar->add_menu( array(
			'parent' => 'user-actions',
			'id'     => 'lost-pass',
			'title'  => __( 'Lost Password', 'bbpress-org-plugin' ),
