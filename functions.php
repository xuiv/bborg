 ...
 * @return bool
 */
function bb_base_is_bbpress_old() {
...
function bb_base_is_bbpress() {
	return (bool) apply_filters( 'bb_base_is_bbpress', true );
}
...
function bborg_active_sidebars(){
	if(is_active_sidebar('bborg-sidebar')){
		return true;	
	}else{
		return false;	
	}
...
add_action('widgets_init', 'bborg_widgets_init');
function bborg_widgets_init() 
{
    register_sidebar(
        array(
        'name'          => __('bborg Widget Area', 'bborg'),
        'id'            => 'bborg-sidebar',
        'description'   => __('Add widgets here to appear in your sidebar.', 'bborg'),
 ...
 * change title 
 */
add_action( 'after_setup_theme', 'bborg_theme_setup' );
function bborg_theme_setup() {
    /*
...
add_filter('document_title_parts', 'bborg_override_post_title', 10);
function bborg_override_post_title($title){
	//$title['title'] .= bloginfo( 'name' );
   return $title; 
}

load_theme_textdomain('bborg');